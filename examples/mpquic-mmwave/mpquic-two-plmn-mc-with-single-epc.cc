...
main(int argc, char* argv[])
{
    ...
    // Install mmWave, lte, mc Devices to the nodes
    NetDeviceContainer lteEnbDevs_dup = mmwaveHelper->InstallLteEnbDevice(lteEnbNodes_dup);
    NetDeviceContainer mmWaveEnbDevs_dup = mmwaveHelper->InstallEnbDevice(mmWaveEnbNodes_dup);
    NetDeviceContainer mcUeDevs_dup = mmwaveHelper->InstallMcUeDevice(ueNodes); 

    Ipv4InterfaceContainer ueIpIface_dup = epcHelper->AssignUeIpv4Address(NetDeviceContainer(mcUeDevs_dup));
    ...
    mmwaveHelper->AddX2Interface(lteEnbNodes_dup, mmWaveEnbNodes_dup);
    mmwaveHelper->AttachToClosestEnb(mcUeDevs_dup, mmWaveEnbDevs_dup, lteEnbDevs_dup);
    //
