...
// : MmWaveHelper(0,1) {}
// MmWaveHelper::MmWaveHelper(uint64_t init_imsi, uint16_t init_cellId)
    : m_imsiCounter(0),
      m_cellIdCounter(1),
      m_harqEnabled(false),
...
NetDeviceContainer
MmWaveHelper::InstallMcUeDevice(NodeContainer c, uint64_t imsi_base)
{
    uint64_t local_imsi = imsi_base;
    NS_LOG_FUNCTION(this);
    Initialize(); // Run DoInitialize (), if necessary
    NetDeviceContainer devices;
    for (NodeContainer::Iterator i = c.Begin(); i != c.End(); ++i)
    {
        Ptr<Node> node = *i;
        NS_LOG_DEBUG("m_imsi current value " << m_imsiCounter);
        Ptr<NetDevice> device = InstallSingleMcUeDevice(node, ++local_imsi);
        device->SetAddress(Mac64Address::Allocate());
        devices.Add(device);
    }
    return devices;
...
NetDeviceContainer
MmWaveHelper::InstallLteEnbDevice(NodeContainer c, uint16_t preferred_rnti_base)
{
    NS_LOG_FUNCTION(this);
    Initialize(); // Run DoInitialize (), if necessary
    NetDeviceContainer devices;
    ...
    {
        Ptr<Node> node = *i;
        Ptr<NetDevice> device = InstallSingleLteEnbDevice(node, preferred_rnti_base);
        device->SetAddress(Mac64Address::Allocate());
        devices.Add(device);
    }
    return devices;
...
Ptr<NetDevice>
MmWaveHelper::InstallSingleMcUeDevice(Ptr<Node> n, uint64_t prefered_imsi)
{
    NS_LOG_FUNCTION(this);
...
Ptr<NetDevice>
MmWaveHelper::InstallSingleLteEnbDevice(Ptr<Node> n, uint16_t preferred_rnti_base)
{
    ...
    Ptr<LteEnbRrc> rrc = CreateObject<LteEnbRrc>(preferred_rnti_base);
    Ptr<LteEnbComponentCarrierManager> ccmEnbManager =
